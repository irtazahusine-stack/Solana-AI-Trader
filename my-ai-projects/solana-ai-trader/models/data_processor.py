...
class DataProcessor:
    ...
    def generate_insights(self, market_data: pd.DataFrame, patterns: List[Dict]) -> List[str]:
        ...
        if avg_change > 2:
            ...
            insights.append("=É Market experiencing bearish pressure with average losses over 2%")
        else:
        ...
        if not high_volume_tokens.empty:
            ...
            insights.append(f"=Ê High trading activity detected in: {tokens}")
        ...
        for pattern in patterns:
            if pattern['type'] == 'support_resistance':
                ...
                if pattern['direction'] == 'upward' and pattern['strength'] > 5:
                    insights.append(f"=È Strong upward trend detected ({pattern['strength']:.1f}% above long-term average)")
